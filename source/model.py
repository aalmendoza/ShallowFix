...
class Model():
	...
	def get_entropy(self, sess, chars, vocab, token_list):
		entropy_list = []
		state = sess.run(self.cell.zero_state(1, tf.float32))
		total_entropy = 0
		for n in range(len(token_list)-1):
	...
	def get_entropy_stats_per_type(self, sess, chars, vocab, token_file, token_type_file):
		...
		for token_type, dist in entropy_map.items():
			...
			q2 = np.percentile(dist, 50)
			q3 = np.percentile(dist, 75)
			entropy_stats_map[token_type] = DistributionStats(mean, sd, q1, q2, q3)
		return entropy_stats_map

	# Consider collapsing into one function with get_entropy_stats_per_type
	def get_entropy_stats(self, sess, chars, vocab, token_file):
		entropy_scores = []
		state = sess.run(self.cell.zero_state(1, tf.float32))
		num_iterations = 100000
		...
		sd = np.std(entropy_scores)
		q1 = np.percentile(entropy_scores, 25)
		q2 = np.percentile(entropy_scores, 50)
		q3 = np.percentile(entropy_scores, 75)
		print("Mean: {0}".format(mean))
		print("Standard Deviation: {0}".format(sd))
		print("Q1: {0}".format(q1))
		print("Q2: {0}".format(q2))
		print("Q3: {0}".format(q3))
		return DistributionStats(mean, sd, q1, q2, q3)

	def get_zscores(self, sess, chars, vocab, token_file, dist_stats):
		entropy_scores = []
		zscores = []
		state = sess.run(self.cell.zero_state(1, tf.float32))
		# num_iterations = 1000
		...
		with open(token_file, 'r') as f:
			...
			while True:
				...
				prob_next_token = prob_dist[vocab[next_token]]
				entropy = -1.0 * np.log2(prob_next_token)
				zscore = (entropy - dist_stats.mean) / dist_stats.sd
				entropy_scores.append(entropy)
				zscores.append(zscore)
				token = next_token
				# print(i)
				i+=1
		return entropy_scores, zscores

	def get_zscores_by_type(self, sess, chars, vocab, token_file, token_type_file, dist_stats_map):
		entropy_scores = []
		zscores = []
		state = sess.run(self.cell.zero_state(1, tf.float32))
		# num_iterations = 1000
		...
		with open(token_file, 'r') as token_f, open(token_type_file, 'r') as token_type_f:
			...
			while True:
				...
				prob_next_token = prob_dist[vocab[next_token]]
				entropy = -1.0 * np.log2(prob_next_token)
				zscore = (entropy - dist_stats_map[next_token_type].mean) / dist_stats_map[next_token_type].sd
				entropy_scores.append(entropy)
				zscores.append(zscore)
